...
def build_worker_prompt(
    ...
    prompt_parts.append(
        ...
        "wrong by default.\n\n"
        "If you cannot back a number with (a), (b), or (c), you have three "
        "choices, in order of preference:\n"
        "  1. call a data tool to fetch it (preferred), or\n"
        "  2. if the data tool fails, returns only metadata, or returns "
        "empty/stale data, call `web_search` to find the real, current figure "
        "(e.g. search \"<ticker> bitcoin holdings 2026\", \"<ticker> latest "
        "stock price\", \"<company> Q2 2026 revenue\"), then cite the source "
        "and as-of date, or\n"
        "  3. if web_search also cannot confirm it, write \"DATA UNAVAILABLE\" "
        "for that item and qualify the statement — do NOT invent a "
        "plausible-looking value.\n\n"
        "NEVER fabricate or guess a number to fill a gap. A missing number "
        "labeled \"DATA UNAVAILABLE\" is correct; an invented number is a "
        "critical failure.\n\n"
